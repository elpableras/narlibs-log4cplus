		...
		stage('Copy Artifacts') { 
			parallel {
				stage('Windows') {                  
					agent { label "windows" }
					steps {
						copyArtifacts filter: 'msvc14/x64/bin.Release/*.*', fingerprintArtifacts: true, flatten: true, optional: true, projectName: ' log4cplus/log4cplus/master', selector: lastSuccessful(), target: 'src/nar/resources/aol/amd64-Windows-msvc/lib'
					}
				}
				stage('Unix') {            
					agent { label "unix" }
					steps {
						copyArtifacts filter: 'include/log4cplus/**/*.*', fingerprintArtifacts: true, flatten: true, optional: true, projectName: ' log4cplus/log4cplus/master', selector: lastSuccessful(), target: 'src/nar/resources/noarch/'
						copyArtifacts filter: '.libs/liblog4cplus-2.0.so', fingerprintArtifacts: true, flatten: true, optional: true, projectName: ' log4cplus/log4cplus/master', selector: lastSuccessful(), target: 'src/nar/resources/aol/amd64-Linux-gpp/lib/'
